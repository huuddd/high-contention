...
import com.example.ticketing.ticket.strategy.NaiveTicketingStrategy;
...
public class StrategyConfig {
    ...
    @Bean
    @ConditionalOnProperty(name = "ticketing.strategy", havingValue = "naive", matchIfMissing = true)
    public TicketingStrategy naiveTicketingStrategy(EventRepository eventRepository,
                                                     SeatRepository seatRepository,
                                                     TicketRepository ticketRepository,
                                                     ConflictMetrics metrics) {
        log.info(">>> Active strategy: NAIVE (intentionally broken — for baseline benchmark only)");
        return new NaiveTicketingStrategy(eventRepository, seatRepository, ticketRepository, metrics);
    }
}
