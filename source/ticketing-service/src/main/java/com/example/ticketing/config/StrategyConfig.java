...
import com.example.ticketing.ticket.strategy.PessimisticTicketingStrategy;
...
public class StrategyConfig {
    ...
    @Bean
    @ConditionalOnProperty(name = "ticketing.strategy", havingValue = "pessimistic", matchIfMissing = true)
    public TicketingStrategy pessimisticTicketingStrategy(EventRepository eventRepository,
                                                           SeatRepository seatRepository,
                                                           TicketRepository ticketRepository,
                                                           ConflictMetrics metrics) {
        log.info(">>> Active strategy: PESSIMISTIC LOCKING (SELECT FOR UPDATE)");
        return new PessimisticTicketingStrategy(eventRepository, seatRepository, ticketRepository, metrics);
    }
}
